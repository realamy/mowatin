...
const SidebarLinkGroup = ({ links, title, border }: { links: SidebarLink[]; title?: string; border?: boolean }) => {
  ...
  return (
    <div className={border ? "my-8 border-t border-border pt-4" : ""}>
      {title ? <h4 className="mb-2 px-2 text-xs uppercase tracking-wider text-muted-foreground">{title}</h4> : null}
      <ul>
        {links.map((link) => (
          <li key={link.title}>
  ...
  return (
    ...
    >
      <div className="flex items-center">
        <div
          className={cn("absolute left-0 h-6 w-[4px] rounded-r-lg bg-primary opacity-0", active ? "opacity-100" : "")}
        />
